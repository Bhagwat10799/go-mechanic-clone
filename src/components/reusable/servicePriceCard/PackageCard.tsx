...
export const PackageCard = ({ data, ...rest }: any) => {
  ...
  const checoutRowData=useSelector((state:any)=> state.checkout)
  ...
  useEffect(() => {
    if (isItemAdded){ dispatch(AddCheckoutData(rowItem));console.log(rowItem.title)}
  }, [isItemAdded]);
  ...
  useEffect(() => {
    if (isItemAdded) console.log(checoutRowData);
  }, [isItemAdded]);
  const firstHandler = () => {
    setOpen(!open);
  };
    ...
    <>
      ...
      >
        ...
        <div className="d-flex justify-content-between">
          ...
          <div>
            {checoutRowData!==null && !checoutRowData.some((item:any)=>item.title== data.title ) ? (
              <button

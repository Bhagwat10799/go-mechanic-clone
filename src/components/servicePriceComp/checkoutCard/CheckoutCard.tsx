...
export const CheckoutCard = ({ selectedCar }: any) => {
  ...
  const carDetailsData= JSON.parse(`${carDetails}`)
  const checkoutData = useSelector((state: any) => state.checkout);
    ...
    <div className={styles["checkout-container"]}>
      ...
      >
        <div className="text-center">
          <img src={`/${carDetailsData && carDetailsData.carImage }`} alt="img" />
        </div>
        <div className="d-flex justify-content-between mx-2">
          <div className="d-flex">
            <h5 className="mx-2">
              { carDetailsData && carDetailsData.brand} {carDetailsData && carDetailsData.carModel}
            </h5>
            <span>{carDetailsData && carDetailsData.fuelType} </span>
          </div>
          ...
          <>
            ...
            <div className={styles["subTotal"]}>
              ...
              <div className={styles["checkout-btn"]}>
                <span>₹{subTotal}</span>
                <span onClick={checkoutHandler}>CHECKOUT</span>
              </div>
            </div>
          </>
        )}
